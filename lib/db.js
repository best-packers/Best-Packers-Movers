  ...
  if (!sqlDbInstance) {
    ...
    if (resolvedPath) {
      try {
        ...
        console.log(`✅ Loaded SQLite database from disk: ${resolvedPath} (${fileBuffer.length} bytes)`);
      } catch (err) {
        console.warn('Could not read disk file, falling back to bundled in-memory data:', err.message);
      }
    }

    // Serverless fallback: load from native JavaScript in-memory bundle
    if (!fileBuffer || fileBuffer.length === 0) {
      try {
        const dbBase64 = require('@/data/dbBase64');
        fileBuffer = Buffer.from(dbBase64, 'base64');
        console.log(`✅ Loaded bundled database directly from in-memory code module (${fileBuffer.length} bytes)`);
      } catch (b64Err) {
        console.error('Failed to load bundled database from memory:', b64Err.message);
      }
    }
    ...
    if (fileBuffer && fileBuffer.length > 0) {
      ...
      console.error('❌ CRITICAL: Database could not be loaded from disk or memory!');
